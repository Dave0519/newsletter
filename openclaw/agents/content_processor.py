...
class ContentProcessor:
    ...
    def _llm_practical_implication(self, title: str, body: str, summary: str) -> str:
        ...
        prompt = (
            ...
            "작성 원칙:\n"
            "1) 기사의 주요 사실(출시, 수익, 감원, 계약, 정책 등)과 직접 연결된 실무적 영향 중심으로 작성한다.\n"
            "2) 기/업/팀 개인이 참고할 수 있는 조치 포인트, 리스크, 전략적 대응 방향을 관찰형 문장으로 정리한다.\n"
            "3) 직접 지시형/목록형(예: 첫째, 둘째, 1) 2)...)보다 자연스러운 문장 흐름으로 작성한다.\n"
            "4) 기사 본문 외부의 일반 상식, 추정, 추측은 배제하고 기사 근거만 사용한다.\n"
            "5) 단문 위주로 3~5문장 이내로 작성한다.\n"
            "출력은 시사점 본문만.\n\n"
            f"[제목]\n{title}\n\n[요약]\n{summary}\n\n[본문]\n{body[:8000]}"
        )
        try:
