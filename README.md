...
운영 관측성 위에 이어서, 개인화 품질 개선도 반영했습니다.

- Stage A에서 고객 `searchQueries` / `needClusters`를 활용한 query-aware upstream pool 형성
- SK hynix 공통 hot topic의 bounded injection (`capPerEdition` 기본값 `2`)
- final selection 직전 overlap-aware rerank 적용
- API `customerDecisions[].phase2`에서 upstream / sharedHotTopic / rerank 결과 확인 가능

관련 문서:

- `OPENCLAW_OBSERVABILITY_CHANGELOG_KO.md`
- `OPENCLAW_PERSONALIZATION_PHASE2_CHANGELOG_KO.md`
