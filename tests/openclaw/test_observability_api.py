...
class ObservabilityApiTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.store.append_customer_decision(
            ...
            selected=[{"title": "sel", "url": "https://example.com/b", "source": "rss", "issue_angle_id": "issue:1", "origin_type": "query_aware_source", "origin_detail": {"query": "HBM"}}],
            gate_ok=True,
            delivery={"email": True},
            soft_warnings=[],
            gate_errors=[],
            phase2={"sharedHotTopic": {"action": "skip", "reason": "low_affinity"}, "rerank": {"reordered": 1}},
        )
    ...
    def test_customer_decisions_expose_phase2_fields(self) -> None:
        status, payload = build_payload(self.store, "/api/customers", {"customer_id": ["c1"]})
        self.assertEqual(status, 200)
        self.assertEqual(payload["items"][0]["phase2"]["sharedHotTopic"]["reason"], "low_affinity")
        self.assertEqual(payload["items"][0]["selected"][0]["originType"], "query_aware_source")
